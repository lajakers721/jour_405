...
```{r}
songs |> summarize(mean_danceability = mean(danceability), (sd_danceability=sd(danceability)))
```
...
A: The distribution of the histogram is slightly skewed to the right. The mean tells us that the average danceability score for all the songs listed was 0.673. The standard deviation tells us that most of the data falls close to the mean as a result of the standard deviation being lower. A story pitch I could generate from the data is how most of songs with a high danceability score are rap or hiphop songs or are by rap or hiphop artists. This was done by looking at the dataset and sorting by highest to lowest. 
