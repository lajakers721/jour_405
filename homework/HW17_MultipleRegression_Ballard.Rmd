...
A: The variables with the two highest correlation scores are income and pct_bachelors_higher as well as pct_white and turnout according to the matrix. Using the dependent variable, turnout, the independent variable with the highest correlation is pct_unemployed but it is a negative correlation whereas the two previous ones for instance were strongly positive. On the other side of the spectrum, median_age adn pct_bachelors_higher have zero correlation whatsoever. Some variables with low correlations include income and pct_white, pct_white and pct_bachelors_higher and pct_unemployed and pct_bachelors_higher. Most have a linear relation but population has the least linear as the points are really scattered around. 
...
A: I would say it does a pretty good. Looking at the multiple r-squared and adjusted r-squared, they are both above 0.8 which is on the hgiher side. The residual standard error is low which is good since it is not deviating from actual turnout much. The f-statistic also points to a statistically significant result. Intercept is highly significant, the income is somewhat significant and median_age and pct_white are a little significant. The rest of the variables are not significant enough to warrant a star. 
...
A: The original model is much more significant from the r-squared values. Between the new models, model 3, which I used pct_bachelors_higher for was the least significant and noticeably so. It is at 0.05044749 and the next lowest is model 2, income, at 0.1656268 and that is pretty close to model 4, median_age, at 0.2111296. With that model 4 is significantly better than model 3 and model 2 is a good bit of significance better than model 3.   
...
A: This could be used to predict election results. Based on where candidates rank in different demographics we can see how those demographics affect voter turnout. Then we can compare the data and say this person will win this group and this will help turn the election or something of that sort.